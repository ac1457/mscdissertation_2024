#!/usr/bin/env python3
"""
Advanced Fusion Implementation - Fixed Version
==============================================
Implementation of next steps plan with proper categorical encoding
"""
...
class AdvancedFusionModel(BaseEstimator, ClassifierMixin):
    ...
    def _optimize_weights(self, predictions, y_true):
        ...
        weight_combinations = [
            ...
            [0.3, 0.25, 0.2, 0.15, 0.1],  # RF and XGB heavy
            [0.25, 0.25, 0.2, 0.15, 0.15],  # Balanced
            [0.4, 0.3, 0.2, 0.1, 0.0],  # Top 3 models
            [0.5, 0.3, 0.2, 0.0, 0.0],  # Top 2 models
        ]
...
class DataPreprocessor:
    """Handle data preprocessing including categorical encoding"""
...
class ExpandedDatasetGenerator:
    ...
    def _generate_diverse_financial_data(self, n_samples):
        ...
        data = {
            ...
            'home_ownership': np.random.choice([0, 1, 2, 3], n_samples),
            'purpose': np.random.choice(range(8), n_samples),
            'age': np.random.normal(35, 10, n_samples),
            'gender': np.random.choice([0, 1, 2], n_samples),
            'race': np.random.choice([0, 1, 2, 3, 4], n_samples),
            ...
            'credit_history_length': np.random.normal(8, 3, n_samples),
            'num_credit_cards': np.random.poisson(3, n_samples),
            'mortgage_accounts': np.random.poisson(0.5, n_samples),
            'auto_loans': np.random.poisson(0.8, n_samples),
            'student_loans': np.random.poisson(0.3, n_samples),
            'other_loans': np.random.poisson(0.2, n_samples),
            'bankruptcy_count': np.random.poisson(0.1, n_samples),
            'foreclosure_count': np.random.poisson(0.05, n_samples),
            'tax_liens': np.random.poisson(0.1, n_samples),
            'collections_count': np.random.poisson(0.3, n_samples),
            'charge_offs': np.random.poisson(0.1, n_samples),
        }
    ...
    def _add_temporal_diversity(self, df):
        ...
        seasons = []
        for month in df['application_month']:
            if month in [12, 1, 2]:
...
class FairnessMonitoringFramework:
    ...
    def calculate_comprehensive_fairness(self, df, y_true, y_pred_proba, threshold=0.5):
        ...
        # Define demographic groups (using encoded values)
        demographic_groups = {
...
def run_advanced_fusion_implementation():
    ...
    print("ADVANCED FUSION IMPLEMENTATION - FIXED VERSION")
    print("="*60)
    ...
    expanded_df_processed = preprocessor.fit_transform(expanded_df)
    
    # 3. Advanced feature engineering
    print("\n3. ADVANCED FEATURE ENGINEERING")
    print("-" * 40)
    
    feature_columns = [col for col in expanded_df_processed.columns if col != 'loan_status']
    X = expanded_df_processed[feature_columns]
    y = expanded_df_processed['loan_status']
    ...
    # 4. Train advanced fusion models
    print("\n4. TRAINING ADVANCED FUSION MODELS")
    print("-" * 40)
    ...
    fairness_results = fairness_framework.calculate_comprehensive_fairness(
        expanded_df_processed.loc[X_test.index], y_test, best_predictions
    )
...
def generate_advanced_fusion_report(fusion_results, fairness_results, df, X):
    ...
    with open('reports/advanced_fusion_implementation_report.txt', 'w') as f:
        ...
        f.write("- Comprehensive fairness monitoring across 5 demographic dimensions\n")
        f.write("- Enhanced feature engineering with 70+ features\n")
