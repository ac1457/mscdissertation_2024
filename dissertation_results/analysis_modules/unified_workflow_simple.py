#!/usr/bin/env python3
"""
Unified Workflow Simple - Lending Club Sentiment Analysis
========================================================
Simplified unified workflow that consolidates all scattered workflow variants.
Demonstrates real cross-validated metrics and consistent modest narrative.
"""
...
class UnifiedWorkflowSimple:
    """
    Simplified unified workflow for Lending Club sentiment analysis
    """
    ...
    def perform_real_cross_validation(self, X, y, cv_folds=5):
        ...
        # Models to evaluate (excluding XGBoost due to compatibility issues)
        models = {
    ...
    def run_comprehensive_analysis(self):
        ...
        """
        print("UNIFIED WORKFLOW - COMPREHENSIVE ANALYSIS")
        print("=" * 50)
        ...
        for result in comprehensive_results:
            if result['Feature_Set'] != 'Traditional':
                ...
                # Calculate bootstrap CI for improvement (simplified)
                # Use the fold results directly for bootstrap
                all_traditional_aucs = traditional['AUC_Folds']
                all_variant_aucs = result['AUC_Folds']
                
                # Bootstrap the difference
                bootstrap_diffs = []
                for _ in range(1000):
                    # Sample with replacement
                    trad_sample = np.random.choice(all_traditional_aucs, len(all_traditional_aucs), replace=True)
                    var_sample = np.random.choice(all_variant_aucs, len(all_variant_aucs), replace=True)
                ...
                t_stat = auc_improvement / (np.std(result['AUC_Folds']) / np.sqrt(len(result['AUC_Folds'])))
                p_value = 2 * (1 - np.abs(t_stat))  # Simplified p-value
    ...
    def generate_unified_report(self, comprehensive_results, improvements):
        """
        Generate unified report with consistent modest narrative
        """
        print("Generating unified report...")
        
        report = []
        report.append("UNIFIED WORKFLOW REPORT - LENDING CLUB SENTIMENT ANALYSIS")
        report.append("=" * 70)
        report.append("")
        ...
        report.append("• Two algorithms: RandomForest, LogisticRegression")
        report.append("")
    ...
    def run_complete_unified_workflow(self):
        """
        Run complete unified workflow
        """
        print("RUNNING UNIFIED WORKFLOW")
        print("=" * 40)
        ...
        report = self.generate_unified_report(comprehensive_results, improvements)
        
        # Save results
        comprehensive_results.to_csv('final_results/unified_workflow_comprehensive_results.csv', index=False)
        improvements.to_csv('final_results/unified_workflow_improvements.csv', index=False)
        
        with open('methodology/unified_workflow_report.txt', 'w') as f:
            f.write(report)
        
        print("✅ Unified workflow complete!")
        print("✅ Saved results:")
        print("  - final_results/unified_workflow_comprehensive_results.csv")
        print("  - final_results/unified_workflow_improvements.csv")
        print("  - methodology/unified_workflow_report.txt")
        
        return comprehensive_results, improvements

if __name__ == "__main__":
    workflow = UnifiedWorkflowSimple()
    results = workflow.run_complete_unified_workflow()
    print("✅ Unified workflow execution complete!") 